...
class Api {
  ...
  setOptions({
    ...
    this.registerAdminEndpoint = registerAdminEndpoint;
  ...
  async registerAdmin(credentials) {
    const { data } = await axios.post(this.registerAdminEndpoint, credentials);
    return data;
  }
  ...
  async activate(id) {
    const { data } = await axios.get(`activate?id=${id}`);
    return data;
  }
}
